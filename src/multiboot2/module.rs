use multiboot2::header::{Tag, TagIter};
...
impl ModuleTag {
    ...
    // as valid utf-8, therefore this funtion produces
    // defined behavior
...
impl Iterator for ModuleIter {
    ...
    fn next(&mut self) -> Option<&'static ModuleTag> {
        self.iter.find(|x| x.typ == 3).map(|tag| unsafe{&*(tag as *const Tag as *const ModuleTag)})
    }
}